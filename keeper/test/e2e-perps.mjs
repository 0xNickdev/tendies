...
//   STATE_DIR=./data-e2e PORT=3344 FEE_TOKEN=0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168 \
//     PERPS_DRYRUN_RESERVE_USD=200 node index.js &      # USDG fee: $1 = 1e6 units
...
if (process.argv[2] === "seed") {
  ...
  state.ledger[fresh.address] = { accrued: String(50n * 10n ** 6n), updatedAt: Date.now() };
  fs.writeFileSync(stateFile, JSON.stringify(state, null, 2));
  fs.writeFileSync(keyFile, JSON.stringify({ privateKey: fresh.privateKey }));
  console.log(`seeded $50 for ${fresh.address} - now start the keeper with STATE_DIR=${stateDir}`);
  process.exit(0);
}

const kp = new ethers.Wallet(JSON.parse(fs.readFileSync(keyFile, "utf8")).privateKey);
...
const closeMessage = (id, ts) => `RobinX perp close\nposition: ${id}\nts: ${ts}`;
...
let r = await post("/perps/open", { owner, ...p, signature: await sign(openMessage(p)) });
...
ts = Date.now();
r = await post("/perps/close", { owner, id, ts, signature: await sign(closeMessage(id, ts), other) });
check("stranger cannot close", !r.ok && r.error === "signature does not match owner", r.error);

ts = Date.now();
r = await post("/perps/close", { owner, id, ts, signature: await sign(closeMessage(id, ts)) });
