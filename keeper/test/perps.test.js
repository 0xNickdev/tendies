...
process.env.PAYOUT_TOKENS =
  "OPENAI*:0x000000000000000000000000000000000000dEaD:10000,TSLA:0x322F0929c4625eD5bAd873c95208D54E1c003b2d:3000";
...
const USDC = 10n ** 6n;
const trader = ethers.Wallet.createRandom();
const owner = trader.address;
const sign = (msg, kp = trader) => kp.signMessage(msg);
...
const open = async (fields, kp = trader) => {
  ...
  body.signature = await sign(openMessage({ ...body, leverage: body.leverage, marginUsd: body.marginUsd }), kp);
  return openPosition(body);
};
const close = async (id, kp = trader) => {
  const ts = Date.now();
  return closePosition({ owner, id, ts, signature: await sign(closeMessage({ id, ts }), kp) });
};

console.log("\n1. Рынки из PAYOUT_TOKENS");
{
  const m = markets();
  check("два рынка", m.length, 2);
  check("OPENAI* — on-chain", `${m[0].market}/${m[0].listed}`, "OPENAI/false");
  check("TSLA — биржевой", `${m[1].market}/${m[1].listed}`, "TSLA/true");
}
...
console.log("\n7. Подписи");
{
  ...
  body.signature = await sign(openMessage(body));
