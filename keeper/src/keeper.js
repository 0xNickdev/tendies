...
// Последний снимок холдеров, чтобы /account не гонял скан Transfer-событий на
...
export const state = {
  ...
  treasury: treasury?.address ?? null,
...
export async function tickEpoch() {
  ...
  try {
    if (!epochDue()) {
      ...
      // The schedule survives a redeploy but the snapshot does not: warm it,
      // or /account shows every wallet at zero until the next epoch.
      if (!snapshot.at) await takeSnapshot();
      return;
    }

    const holders = await takeSnapshot();
    if (!holders.length) {
    ...
    // ── 1. accrue ────────────────────────────────────────────────────────
    // Pull the creator share out of the Pons locker first, so the fee this
    // epoch counts is the fee this epoch earned.
    if (!config.dryRun) {
      try {
        await collectPonsFees();
      } catch (e) {
        log.warn(`pons collect failed (${e.shortMessage || e.message}) — accruing what is already here`);
      }
    }
    ...
    if (!config.dryRun) {
      const ethBefore = await ethBalance();
      const recipients = groups.reduce((n, g) => n + g.owners.length, 0);
      // one transfer per recipient plus a swap per group, at the current gas price
      const gasUnits = BigInt(recipients * config.transferGas + groups.length * 400_000);
      const worstCase = Number(gasUnits * (await gasPriceWei().catch(() => 0n))) / 1e18;
      if (ethBefore < worstCase) {
        log.warn(
          `treasury has ${ethBefore.toFixed(5)} ETH but this epoch could need up to ` +
            `${worstCase.toFixed(5)} for ${recipients} recipients - top it up, ` +
    ...
    for (const group of groups) {
      ...
      try {
        const swap = await swapFeeInto(group, group.total);
        ...
        await payGroup(
          { ...group, symbol: `${group.symbol}→fee`, address: config.feeToken },
          group.total,
          epoch,
          recordPayout,
        );
    ...
    if (treasury && eth < config.minEthWarn) {
      log.warn(`treasury ETH low: ${eth} — it pays gas for every swap and payout`);
    }
  } catch (e) {
