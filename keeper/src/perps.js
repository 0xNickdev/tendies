...
// them to every holder pro-rata, so the pool of ROBX holders collectively
...
// signs a short message (EIP-191), the keeper recovers the signer.
...
import { canonicalOwner, verifySigned } from "./choice.js";
import {
  accruedOf,
  addAccrual,
  adjustReserve,
...
export async function openPosition(body) {
  const { market, side, leverage, marginUsd, ts, signature } = body ?? {};
  const owner = canonicalOwner(body?.owner);
